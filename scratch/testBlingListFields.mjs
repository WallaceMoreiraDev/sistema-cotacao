...
async function run() {
  ...
  const resCampos = await fetch('https://www.bling.com.br/Api/v3/produtos?limite=1&campos=fornecedor,camposCustomizados', {
    headers: {
      'Authorization': `Bearer ${token}`
    }
  });
  ...
  console.log("\nWith params:");
  console.log(JSON.stringify(await resCampos.json(), null, 2));
}
