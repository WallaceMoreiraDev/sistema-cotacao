  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="rounded-[32px] border border-slate-200/60 bg-white/70 backdrop-blur-xl p-6 sm:p-8 shadow-2xl shadow-slate-200/40 relative overflow-hidden flex flex-col min-h-[500px]">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 border-b border-slate-200/80 pb-6">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500 max-w-xl">
              Essas categorias aparecem para os vendedores. As famílias são <strong>sincronizadas com as Categorias do Bling</strong>. Ao criar/editar aqui, a categoria é criada/editada lá. 
            </p>
          </div>
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <div className="relative w-full sm:w-64">
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm text-slate-800 outline-none transition-all placeholder:text-slate-400 focus:border-[#F7C00C] focus:ring-4 focus:ring-[#F7C00C]/10"
                />
        ...
        {/* Alerta de Exclusão */}
        <div className="relative z-10 mb-6 flex items-start gap-3 rounded-2xl bg-red-50/50 p-4 border border-red-100">
          <svg className="h-5 w-5 text-red-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
          <div>
            <h4 className="text-sm font-bold text-red-900">Atenção ao Excluir</h4>
            <p className="text-xs text-red-700/90 mt-1">
              A exclusão de uma família aqui no sistema <strong>NÃO</strong> exclui a categoria lá no Bling automaticamente. O Bling bloqueia exclusões se houver produtos atrelados. Caso precise excluir definitivamente, apague primeiro no painel do Bling e depois aqui.
            </p>
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {families.filter(f => f.name.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 ? (
                    <tr>
                      <td colSpan={3} className="px-6 py-8 text-center text-slate-400">
                        Nenhuma família encontrada para "{searchQuery}".
                      </td>
                    </tr>
                  ) : (
                    families.filter(f => f.name.toLowerCase().includes(searchQuery.toLowerCase())).map((f) => (
